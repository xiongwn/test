...
let main
// 每个配置都跑一遍所有衣服
for (let k = 0; k < bedConfig.length; k++) {
    let currentBedConfig = bedConfig[k]
    // 物料号
    let materialCode = currentBedConfig.materialCode
    // 裁剪工具 1机器 2人工
    let cutType = currentBedConfig.cutType
    // 裁剪方式 1半件 2整件
    let cutMethod = currentBedConfig.cutMethod
    // 最高层数
    let maxLevel = isSecondTime ? Infinity : currentBedConfig.maxLevel
    // 裁剪方式如果是半件，每个衣服占用2层
    if (cutMethod === 1) {
        maxLevel /= 2
    }
    // 最大套裁数
    let maxNum = currentBedConfig.maxNum
    // 最小套裁数
    let minNum = Math.ceil(maxNum / 2)
    // 限制偶数套裁数 1是
    let evenLimit = currentBedConfig.evenLimit
    // abc/y状态 合并1 分开2
    let status = currentBedConfig.status
    // 样片距离
    let distance = currentBedConfig.distance
    // 特提范围[{code,max,min}]
    let changeLimit = currentBedConfig.changeLimit
    ...
    if (cutType === 2 && changeLimit && changeLimit.length > 0) {
        for (let j = 0; j < changeLimit.length; j++) {
            ...
            // 设置了该特体code的衣服
            let d_product = product.filter(e => e.changeArr.some(sub => sub.code === changeLimit[j].code))
            d_product.forEach(e => {
        ...
        if (cutType === 1) {
            ...
            for (let j = Math.min(sortedGroupList[0].length, maxLevel); j > 0; j--) {
                ...
                if (evenLimit === 1
                    ...
                    heightList = heightList.filter(e => e > 0)
                }
                // 如果所有列数加到一起都无法满足最小值，则降一层
                if (heightList.reduce((pre, next) => pre + next, 0) < maxNum && j > 1 && sortedGroupList.some(e => e.length > 1)) {
                    continue
                }
                console.log("sortedGroupList", sortedGroupList, j)
                ...
                for (let k = 0; k < heightList.length; k++) {
                    ...
                    if (maxNum - rowNum >= heightList[k]) {
                        // 赋值床号，列号
                        d_product = sortedGroupList[k].slice(0, heightList[k] * j).map((e, index) => {
                            e.bedIndex = bedIndex
                        ...
                        // 赋值床号，列号
                        d_product = sortedGroupList[k].slice(0, (maxNum - rowNum) * j).map((e, index) => {
                            e.bedIndex = bedIndex
                            e.rowIndex = rowNum + Math.ceil((index + 1) / j) - 1
                            e.materialCode = materialCode
                            return e
                    ...
                    // 修改groupMap
                    groupMap[d_product[0].groupUnicId] = groupMap[d_product[0].groupUnicId].filter(e => !d_product.some(sub => sub.uuid === e.uuid))
                    // 数量到达最大列数跳出
                    if (rowNum === maxNum) {
                        break
                    }
        ...
        // -------------------人工------------------
            ...
            for (let j = 0; j < finalRowList.length; j++) {
                ...
                if (finalRowList[j].rowNum > 0 && maxNum - rowNum >= finalRowList[j].rowNum) {
                    d_product = sortedGroupList.find(e => e[0].groupUnicId === finalRowList[j].groupUnicId).slice(0, finalRowList[j].rowNum * maxLevel).map((e, index) => {
                        e.bedIndex = bedIndex
                    ...
                    d_product = sortedGroupList.find(e => e[0].groupUnicId === finalRowList[j].groupUnicId).slice(0, (maxNum - rowNum) * maxLevel).map((e, index) => {
                        e.bedIndex = bedIndex
                        e.rowIndex = rowNum + Math.ceil((index + 1) / maxLevel) - 1
                        e.materialCode = materialCode
                        return e
                    ...
                    // 成列的已经被排了
                    d_product = arr.slice(arr.length - finalRowList[j].left).map((e, index) => {
                        e.bedIndex = bedIndex
                        e.rowIndex = rowNum + Math.ceil((index + 1) / maxLevel) - 1
                        e.materialCode = materialCode
                        return e
    ...
    currentBedConfig.result = product.map(e => ({
        uuid: e.uuid,
        数据ID: e.数据ID,
        bedIndex: e.bedIndex,
        rowIndex: e.rowIndex
...
if (materialConfig && materialConfig.some(i => i.relateToMain === 1)) {
    main = bedConfig[0]
    let arr = materialConfig.filter(i => i.relateToMain === 1)
    let unicArr = Array.from(new Set(arr.map(i => (i.groupTag || "") + "@" + i.materialCode)))
    const bedIndexSize = new Set(main.result.map(i => i.bedIndex)).size
    for (let i = 0; i < unicArr.length; i++) {
        const split = unicArr[i].split("@")
        const materialCode = split[1]
        const groupTag = split[0]
        let obj = JSON.parse(JSON.stringify(main))
        // console.log("main.result", main.result)
        // console.log("obj.result", obj.result)
        obj.materialCode = materialCode
        obj.relateToMain = 1
        obj.groupTag = groupTag || null
        obj.result = obj.result.map(sub => {
            sub.bedIndex = sub.bedIndex + bedIndex
            return sub
        })
        bedConfig.push(obj)
